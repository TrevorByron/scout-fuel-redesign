...
function SidebarInset({ className, ...props }: React.ComponentProps<"main">) {
  return (
    <main
      ...
      className={cn(
        "relative flex w-full min-h-0 flex-1 flex-col md:peer-data-[variant=inset]:m-2 md:peer-data-[variant=inset]:ml-0 md:peer-data-[variant=inset]:rounded-xl md:peer-data-[variant=inset]:shadow-sm md:peer-data-[variant=inset]:peer-data-[state=collapsed]:ml-2",
        className
      )}
      {...props}
    />
    ...
    props: mergeProps<"button">(
      {
        ...
        onClick: (event: React.MouseEvent<HTMLButtonElement>) => {
          if (
            isMobile &&
            (event.currentTarget as HTMLElement).closest("a[href]")
          ) {
            setOpenMobile(false)
          }
        },
